...
//    }
    ...
    public void setUseSGDThreshold(boolean useSGDThreshold) {
        this.useSGDThreshold = useSGDThreshold;
    }
...
//                rebuildIfAboveMemLimit();
    ...
    private boolean rebuild() {
        ...
        double newThreshold;
        if (useSGDThreshold) {
            newThreshold = computeNewThresholdSGD();
        } else {
            newThreshold = computeNewThreshold(leafListStart, root.getDistThreshold());
        }
//        double newThreshold = computeNewThreshold(leafListStart, root.getDistThreshold());
//        double newThreshold = computeNewThresholdSGD();
        // System.out.println("############## New Threshold = " + newThreshold);

        CFTree newTree = this.rebuildTree(root.getMaxNodeEntries(), newThreshold, root.applyMergingRefinement(), false);
        copyTree(newTree);
    ...
    public double computeNewThresholdSGD() {
        CFEntry newE = new CFEntry();
        for (CFEntry e : root.getEntries()) {
            newE.update(e);
        };
        history.insertData(instanceIndex, newE.getRadius(), root.getDistThreshold());

        long newNumberOfPoints = instanceIndex * 2;
        double expansionFactor = max(1.0, history.predictRadius(newNumberOfPoints) / newE.getRadius());
        double newThreshold = history.predictThreshold(newNumberOfPoints);

        if (max(0, expansionFactor * newThreshold) < root.getDistThreshold()) {
            newThreshold = root.getDistThreshold()
                    * pow(newNumberOfPoints / (double) instanceIndex, 1 / (double) numDims);
        }
        return newThreshold;
    ...
    public void decay() {
        ...
        while (l != null) {
            if (!l.isDummy()) {
                ...
                l.getEntries().removeAll(removeEntries);
                if (l.getEntries().isEmpty()) {
                    //remove leaf and adapt leaf path. Propergate to upper nodes to check if they need to be removed
                    rebuild();
                }
            }
            l = l.getNextLeaf();
